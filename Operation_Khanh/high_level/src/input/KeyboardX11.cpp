...
void KeyboardX11::Start() {
    // Bỏ bàn phím X11 khi chạy chế độ dev_no_keyboard hoặc không có $DISPLAY
...
InputCommand KeyboardX11::GetCommand() {
    InputCommand cmd;
    // isActive nếu cửa sổ X11 đang mở
    cmd.is_active = display_ok_.load();

    // Watchdog X11: quá lâu không có event -> nhả phím di chuyển
...
void KeyboardX11::ThreadFn() {
    ...
    // Yêu cầu WM cấp focus (fix NoMachine)
    XWMHints wm_hints = {};
    ...
    display_ok_ = true; // nguồn bàn phím đã sẵn sàng
    ...
    display_ok_ = false; // nguồn bàn phím dừng
    XFreeGC(display, gc);
