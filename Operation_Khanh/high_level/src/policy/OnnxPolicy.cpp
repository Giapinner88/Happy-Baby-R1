...
void OnnxPolicy::Load(const std::string& model_path, Ort::Env& env,
    ...
    if (obs_size_ != expected_obs_size) {
        throw std::runtime_error("Model " + model_path + " có input size " +
                                 std::to_string(obs_size_) + ", yêu cầu " +
    ...
    std::cout << "[OnnxPolicy] Đã nạp " << model_path << " (obs=" << obs_size_
              << ", metadata=" << (has_metadata_ ? "có" : "KHÔNG") << ")\n";
}
...
bool OnnxPolicy::ReadMetadataArray(const char* key,
    ...
    try {
        ...
        if (i != spec::kNumJoints) {
            std::cerr << "[OnnxPolicy] Metadata '" << key << "' chỉ có " << i << "/"
                      << spec::kNumJoints << " giá trị -> giữ fallback.\n";
            return false;
        }
        out = tmp;
        return true;
