...
// phần overlay tay của ArmGesturePlayer (Update/Active/Weight/BlendInto/MaskObs/
// LeanPitchProxy) để Application/sim chọn nguồn theo ưu tiên rồi gọi y hệt.
//
// TWIN FILE: bản y hệt ở HB/high_level_2/src/input/TeleopReceiver.hpp
...
#pragma pack(pop)
    static_assert(sizeof(Packet) == 60, "UTL1 packet ABI must stay exactly 60 bytes");
    ...
    void Start() {
        ...
        // motor target; tuyệt đối không mở cổng điều khiển này ra LAN.
    ...
    void Update(float dt, bool armed = true) {
        ...
        const bool fresh = armed && have && p.enable &&
                           (now_ns - recv_ns) < static_cast<int64_t>(timeout_ms_ * 1e6f);
        // Weight chỉ biểu diễn quyền của KÊNH TAY. Gói head-only không được làm
        // tay có gain hoặc khiến lần arm_valid kế tiếp nhảy thẳng lên weight=1.
        const bool fresh_arm = fresh && p.arm_valid;

        // Weight ramp/retract của tay.
        if (fresh_arm) weight_ = std::min(1.0f, weight_ + dt / blend_in_s_);
    ...
    bool  Active() const { return weight_ > 0.0f || head_valid_; }
    bool  ArmValid() const { return weight_ > 0.0f; }
    ...
    bool  HeadValid() const { return head_valid_; }
...
private:
    ...
    bool  enabled_    = false;
    int   port_       = 0;
    float timeout_ms_ = 300.0f;
    float blend_in_s_ = 0.4f;
    float retract_s_  = 0.5f;
    float smooth_hz_  = 8.0f;
    float head_yaw_max_   = 1.0f;
    float head_pitch_max_ = 0.6f;
