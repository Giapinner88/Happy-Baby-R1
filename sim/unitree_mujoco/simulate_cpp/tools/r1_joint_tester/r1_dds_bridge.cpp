...
// Canonical R1 logical joint -> IDL mapping shared with the policy stack.
inline constexpr auto joint_idx = R1JointMap::kLogicalToIdl;

// Align EXACTLY with unitree_sdk2/example/r1/high_level/r1_command_sender.cpp default gains
...
void PublisherLoop(ChannelPublisherPtr<LowCmd_> publisher) {
    ...
    // Define max velocities (rad/s) for safety
    ...
    while (running) {
        ...
        // On enable transition, initialize commanded_q to physical current angles
        ...
        if (enabled) {
            for (int i = 0; i < 26; i++) {
                ...
                // Check if kps/kds are valid (not exactly 0 if user wants default, but Python should send default)
...
void HandleExit(int sig) {
    ...
    // Send a dummy command to break recvfrom blocking read
...
int main(int argc, char** argv) {
    ...
    // Initialize target Kp and Kd arrays with default safe values (in case Python doesn't send them yet)
    for (int i = 0; i < 26; i++) {
    ...
    // Explicitly reset DDS entities to stop callbacks and clean up DDS before closing sockets
