"""Neural modules for privileged teacher, adaptation, and deploy selection."""
...
from .config import RmaMetaCfg, RmaMetaModelCfg
...
class EnvironmentFactorEncoder(nn.Module):
  """Train-only encoder ``mu(e_t) -> z_t`` updated jointly by meta PPO."""

  def __init__(self, cfg: RmaMetaModelCfg):
    super().__init__()
    self.cfg = cfg
    self.network = _mlp(
      cfg.factor_dim,
...
class MetaActorCritic(nn.Module):
  """Categorical high-level policy over frozen experts plus HOLD."""

  def __init__(self, cfg: RmaMetaCfg):
    super().__init__()
    self.cfg = cfg
    input_dim = (
      cfg.model.observation_dim + cfg.model.latent_dim + len(cfg.expert_names)
    )
    feature_dim = cfg.model.meta_hidden_dims[-1]
    prefix_dims = cfg.model.meta_hidden_dims[:-1]
    self.backbone = _mlp(input_dim, prefix_dims, feature_dim)
  ...
  def forward(
    ...
    if observation.shape[-1] != self.cfg.model.observation_dim:
      raise ValueError("meta observation dimension does not match config")
    if latent.shape[-1] != self.cfg.model.latent_dim:
      raise ValueError("meta latent dimension does not match config")
    if previous_expert_one_hot.shape[-1] != len(self.cfg.expert_names):
      raise ValueError("previous expert one-hot dimension does not match registry")
...
class AdaptationModule(nn.Module):
  """Deployable recurrent estimator ``phi(history) -> z_hat``."""
  ...
  def set_observation_normalizer(
    ...
    if mean.shape != (self.cfg.observation_dim,):
      raise ValueError("adaptation observation mean has the wrong shape")
    if std.shape != (self.cfg.observation_dim,):
      raise ValueError("adaptation observation std has the wrong shape")
  ...
  def forward(
    ...
    if history.shape[-1] != self.cfg.observation_dim:
      raise ValueError("adaptation observation dimension does not match config")
...
class DeployAdapter(nn.Module):
  """ONNX wrapper exporting only the fixed-history adapter output."""
...
class DeployMetaSelector(nn.Module):
  """ONNX wrapper exporting deploy probabilities without the critic value."""
