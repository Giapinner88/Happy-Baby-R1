...
// Quản lý loa phát nhạc và giọng nói thông báo
class MusicPlayer {
public:
    MusicPlayer() = default;
    ~MusicPlayer() {
    ...
    void InitAudio() {
        try {
            ...
            fprintf(stderr, "[MusicPlayer] AudioClient initialization failed: %s\n", e.what());
            ready_ = false;
        }
    }
    ...
    void SetProjectRoot(std::string root) { project_root_ = std::move(root); }
    ...
    void Play(const std::string& path, float vol01) {
        Enqueue(kMusic, ResolvePath(path), vol01, 0);
    }

    // Phát giọng nói (nếu là đường dẫn file sẽ phát file, ngược lại dùng TTS)
    void Say(const std::string& text_or_path, float vol01, int speaker_id) {
        if (text_or_path.empty()) return;
        Enqueue(kVoice, ResolvePath(text_or_path), vol01, speaker_id);
    }
...
private:
    ...
    std::string ResolvePath(const std::string& value) const {
        if (value.empty() || value.front() == '/' || project_root_.empty()) return value;
        const std::string candidate = project_root_ + "/" + value;
        return FileExists(candidate) ? candidate : value;
    }

    void NotifyBusy(bool busy) {
        if (busy_.exchange(busy) == busy) return;
        if (state_callback_) state_callback_(busy);
    }

    void BeginHighAudio() {
        if (!ready_) return;
        // Báo BUSY trước, chờ gate khóa mic/loa voice rồi mới cắt app và phát hb_audio.
        const bool was_busy = busy_.load();
        NotifyBusy(true);
        if (!was_busy) std::this_thread::sleep_for(std::chrono::milliseconds(60));
        client_->PlayStop(kVoiceApp);
    }

    void PlayVoice(const std::string& text_or_path, float vol, int speaker, uint64_t mygen) {
        if (!ready_) return;
        if (FileExists(text_or_path)) {
            ...
            const int estimated_ms = std::max(1200, static_cast<int>(text_or_path.size()) * 70);
            SleepAbortable(estimated_ms, mygen);
            NotifyBusy(false);
        }
    }
    ...
    void StreamFile(const std::string& path, float vol, uint64_t mygen) {
        if (!std::ifstream(path).good()) {
            fprintf(stderr, "[MusicPlayer] Audio file not found: %s\n", path.c_str());
            return;
        }
        BeginHighAudio();
        float vc = vol < 0.0f ? 0.0f : (vol > 1.0f ? 1.0f : vol);
        ...
        int fds[2];
        if (pipe(fds) != 0) { NotifyBusy(false); return; }
        pid_t pid = fork();
        if (pid < 0) {
            close(fds[0]);
            close(fds[1]);
    ...
    static constexpr const char* kVoiceApp = "pipecat";
};
