...
// Bộ phát hiện ngã (Fall Detector) có debounce tránh spike nhiễu
class FallDetector {
public:
    void Configure(const Tuning& tuning, float dt) {
        ...
        // Lớp an toàn tốc độ góc khớp (ngoài độ nghiêng): vung loạn -> ngã.
    ...
    // Kiểm tra góc nghiêng, vận tốc góc thân, và tốc độ góc khớp (max|dq|) để phát hiện ngã/
    // vung loạn. max_joint_speed = max|dq| trên toàn bộ khớp, do caller tính sẵn (kèm chỉ số
    // khớp worst_joint để ghi log; -1 nếu không có).
