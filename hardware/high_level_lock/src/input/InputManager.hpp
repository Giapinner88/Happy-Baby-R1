...
// Bộ trộn và làm mượt lệnh điều khiển từ Gamepad và Bàn phím
class InputManager {
public:
    ...
    InputCommand GetMergedCommand() {
        ...
        m.want_zero_torque    = g.want_zero_torque    || k.want_zero_torque;
        if (!m.want_emergency_stop) {
            ...
            m.want_get_up_down        = g.want_get_up_down || k.want_get_up_down;
            m.want_gesture            = g.want_gesture ? g.want_gesture : k.want_gesture;
            m.want_teleop_toggle      = g.want_teleop_toggle || k.want_teleop_toggle;
        }
        ...
        // Áp dụng giới hạn tăng/giảm tốc độ lệnh.
        auto slew = [](float cur, float target, float accel, float decel, float dt) {
            float rate = (std::abs(target) < std::abs(cur)) ? decel : accel;
        ...
        vx_ = slew(vx_, raw_vx, t.cmd_accel_vx, t.cmd_decel_vx, dt);
        vy_ = slew(vy_, raw_vy, t.cmd_accel_vy, t.cmd_decel_vy, dt);
        yaw_ = slew(yaw_, raw_yaw, t.cmd_accel_yaw, t.cmd_decel_yaw, dt);
