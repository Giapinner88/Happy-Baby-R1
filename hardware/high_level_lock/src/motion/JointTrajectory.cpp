...
void JointTrajectory::Load(const std::string& npz_path) {
    ...
    if (it_hp != npz.end()) {
        ...
        if (hp.shape.size() != 2 || hp.shape[0] != static_cast<size_t>(num_frames_) ||
            hp.shape[1] != 2 || hp.word_size != sizeof(float)) {
            throw std::runtime_error("head_pos shape must be (frames, 2) float32: " + npz_path);
        }
        const float* hp_data = hp.data<float>();
        head_pos_.resize(static_cast<size_t>(num_frames_));
        for (int f = 0; f < num_frames_; ++f) {
            head_pos_[static_cast<size_t>(f)][0] = hp_data[static_cast<size_t>(f) * 2];
            head_pos_[static_cast<size_t>(f)][1] = hp_data[static_cast<size_t>(f) * 2 + 1];
        }
        has_head_ = true;
    }
