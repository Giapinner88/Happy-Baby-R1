...
`../../docs/teleop/r1_quest3_teleop_hardware.md`. Đường direct-lowcmd cũ
đã bị loại; `teleop.hardware.run_teleop` hiện chỉ đọc `rt/lowstate`.

## Nguồn gốc

Logic IK/mapping là của workspace `teleop/r1/`. Package này **không**
...
Method record: `docs/teleop/05_project_solvers_and_evidence.md`
...
Từ root workspace có một lệnh chạy preflight package rồi copy:

```bash
make teleop-hardware-prepare
```

Lệnh này **không chạy teleop trên robot**. Nó preflight và copy package;
không install/start/enable service, không arm motor và không tạo publisher.

## Bộ giải trên đường phần cứng

```bash
make teleop-hardware HOST_IP=<IP-workstation> ROBOT=<user@IP-robot>
```

Đường active chỉ dùng bộ giải vendor `R1_A5_ArmIK` chạy nguyên xi, cùng bộ giải và cùng
...
được *đo*. Trên phần cứng thì phải *chặn*, nên trần 1.0 rad/s và 2.0 rad/s²
được áp tại đây. Hạ thêm bằng
`--max-joint-velocity-rad-s` / `--max-joint-acceleration-rad-s2`.

Sau pipeline, launcher tải `metadata.json` và `samples.jsonl` từ robot vào cùng
run workstation, rồi tự sinh `metrics.json`, `figures/`, video telemetry và
`artifact_manifest.json`. Video này là animation target/encoder đồng bộ, không
phải camera footage; thiếu bất kỳ nhóm artifact nào làm lệnh trả lỗi.

## An toàn

Không vận hành robot thật khi chưa thỏa điều kiện trong
`docs/safety/safety_rules.md`: có người giữ E-stop, không chạy
