...
# DDS WiFi compatibility test endpoint for the Ubuntu 20.04 workstation.
# Run this at the same time as test/dds_wifi_laptop_ubuntu22.sh on the laptop.

ROS_SETUP="${ROS_SETUP:-/opt/ros/foxy/setup.bash}"
ROS_DOMAIN_ID="${ROS_DOMAIN_ID:-42}"
DURATION_SEC="${DURATION_SEC:-60}"
WIFI_INTERFACE="${WIFI_INTERFACE:-}"
CYCLONEDDS_TMP="${CYCLONEDDS_TMP:-/tmp/hb_cyclonedds_wifi_ubuntu20.xml}"

if [[ ! -f "$ROS_SETUP" ]]; then
  echo "ROS setup file not found: $ROS_SETUP" >&2
  echo "Install/source ROS 2 Foxy or set ROS_SETUP=/path/to/setup.bash" >&2
  exit 2
fi
...
export HB_DDS_ROLE="ubuntu20_workstation"
export HB_DDS_PUB_TOPIC="/hb/dds_wifi/ubuntu20_to_ubuntu22"
export HB_DDS_SUB_TOPIC="/hb/dds_wifi/ubuntu22_to_ubuntu20"
export HB_DDS_DURATION_SEC="$DURATION_SEC"

echo "=== Happy Baby DDS WiFi endpoint: Ubuntu 20.04 workstation ==="
...
def main() -> int:
    ...
    print(f"SUMMARY role=ubuntu20_workstation sent={sent} received={received}")
