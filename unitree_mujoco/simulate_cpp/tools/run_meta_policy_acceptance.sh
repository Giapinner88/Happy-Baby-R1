...
mode="${1:-full}"
output_dir="${2:-/tmp/meta_policy_acceptance_$(date +%Y%m%d_%H%M%S)}"
...
run_case() {
  ...
  echo "[META_POLICY_ACCEPTANCE] ${cases}: scene=${scene} angle=${angle} " \
       "category=${category} direction=${direction} cmd=(${vx},${vy},${yaw})"
  set +e
  if [[ "${META_POLICY_VERBOSE:-0}" == "1" ]]; then
    "${script_dir}/run_sim.sh" meta_policy "${scene}" \
  ...
  echo "[META_POLICY_ACCEPTANCE] control=${result} process=${process_result} exit=${status} log=${log}"
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${scene}" "${angle}" "${category}" "${direction}" \
    "${vx}" "${vy}" "${yaw}" "${result}" "${process_result}" \
    "${status}" "${log}" "${csv}" \
...
echo "[META_POLICY_ACCEPTANCE] cases=${cases} control_failures=${control_failures} process_errors=${process_errors} summary=${summary}"
if [[ "${mode}" == "list" ]]; then
  echo "[META_POLICY_ACCEPTANCE] manifest only; no simulator was started"
  exit 0
fi
