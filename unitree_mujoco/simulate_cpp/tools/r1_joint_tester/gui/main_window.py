...
from PySide6.QtCore import Qt, QTimer, Signal, QEvent, QCoreApplication
...
class MainWindow(QMainWindow):
    ...
    def _update_ui(self):
        ...
        real_vel = state.motor_states[sdk_idx].dq
        tgt_vel = 0.0  # Calculate from target delta
        torque = state.motor_states[sdk_idx].tau
        temp = state.motor_states[sdk_idx].temp

        self.telemetry_graph.update_telemetry(
            self.selected_joint, real_vel, tgt_vel, torque, temp
        )
    ...
    def keyPressEvent(self, event):
        ...
        elif key == Qt.Key_Return or key == Qt.Key_E:
        ...
        # Continuous (W/S - hold)
        elif key == Qt.Key_W and self.motors_enabled:
            self._adjust_angle(self.selected_joint, self.speed_dps * 0.05)
        elif key == Qt.Key_S and self.motors_enabled:
            self._adjust_angle(self.selected_joint, -self.speed_dps * 0.05)
    ...
    def keyReleaseEvent(self, event):
        ...
        # When releasing W or S (continuous control keys), snap target to actual to stop movement immediately
