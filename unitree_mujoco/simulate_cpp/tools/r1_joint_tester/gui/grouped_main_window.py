"""Grouped Main Window - Diagnostic Dashboard"""
import sys
import math
import numpy as np
...
from .grouped_joint_panel import GroupedJointPanel
from .system_power_panel import SystemPowerPanel
...
class GroupedMainWindow(QMainWindow):
    """Main window for R1 Diagnostic Grouped GUI"""
    ...
    def _setup_ui(self):
        self.setWindowTitle("R1 Diagnostic Dashboard - Grouped View")
        ...
        self.panel_lower = GroupedJointPanel("Lower Body", lower_cols)
        self.panel_lower.jointSelected.connect(self._on_joint_selected)
        self.tabs.addTab(self.panel_lower, "Lower Body (Legs)")
        ...
        self.panel_upper = GroupedJointPanel("Upper Body", upper_cols)
        self.panel_upper.jointSelected.connect(self._on_joint_selected)
        self.tabs.addTab(self.panel_upper, "Upper Body (Arms & Head)")

        # System & Power Tab
        self.panel_sys = SystemPowerPanel()
        self.tabs.addTab(self.panel_sys, "System & Power")
    ...
    def _update_ui(self):
        ...
        self.panel_lower.update_all_joints(state.motor_states, self.udp_client.target_q)
        self.panel_upper.update_all_joints(state.motor_states, self.udp_client.target_q)
        self.panel_sys.update_state(state)
    ...
    def _on_joint_selected(self, joint_id: int):
        ...
        # Ensure only one is selected across tabs
        if self.tabs.currentWidget() == self.panel_lower:
            self.panel_upper.deselect_all()
        else:
            self.panel_lower.deselect_all()
