...
std::vector<float> FloatMetadata(Ort::Session& session, const char* key) {
    ...
    for (const auto& token : Split(MetadataValue(session, key))) {
        ...
        if (!std::isfinite(value))
            throw std::runtime_error(std::string(key) + " contains a non-finite value");
...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cerr << "usage: flat_policy_contract_test POLICY.onnx\n";
        return 2;
    }

    try {
        Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "flat_policy_contract_test");
        Ort::SessionOptions options;
        Ort::Session session(env, argv[1], options);

        Require(session.GetInputCount() == 1, "Flat policy must have exactly one input");
        Require(session.GetOutputCount() == 1, "Flat policy must have exactly one output");
        const auto input_shape = session.GetInputTypeInfo(0)
                                     .GetTensorTypeAndShapeInfo().GetShape();
        const auto output_shape = session.GetOutputTypeInfo(0)
                                      .GetTensorTypeAndShapeInfo().GetShape();
        Require(input_shape.size() == 2 && input_shape[1] == 83,
                "Flat policy input must be [batch,83]");
        Require(output_shape.size() == 2 && output_shape[1] == 24,
                "Flat policy output must be [batch,24]");
        ...
        std::cout << "flat_policy_contract_test: PASS input=83 output=24 metadata=complete\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "flat_policy_contract_test: FAIL: " << e.what() << '\n';
        return 1;
    }
}
