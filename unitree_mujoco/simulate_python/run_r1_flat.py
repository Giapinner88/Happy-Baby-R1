...
ACTION_SCALE = np.array([
    ...
    0.125, 0.22,                              # Eo (waist_roll=0.125, waist_yaw=0.22)
...
def main():
    ...
    if len(sys.argv) < 2:
        print(">>> Chạy ở chế độ GIẢ LẬP (Domain 1, Interface 'lo')")
        ChannelFactoryInitialize(1, "lo") 
    else:
        print(f">>> Chạy ở chế độ ROBOT THẬT (Domain 0, Interface '{sys.argv[1]}')")
