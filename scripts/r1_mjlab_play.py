#!/usr/bin/env python3
"""Workspace-safe launcher for Unitree RL MJLab play.py."""
...
UPSTREAM_PLAY = ROOT / "third_party" / "unitree_rl_mjlab" / "scripts" / "play.py"
...
def _patch_r1_robot_asset() -> None:
    ...
    for path in (ROOT / "training", MJLAB_ROOT):
        path_str = str(path)
        if path_str not in sys.path:
            sys.path.insert(0, path_str)

    from happy_baby_r1_training import mjlab_r1_robot_cfg
    import src.assets.robots as robots
    import src.assets.robots.unitree_r1.r1_constants as r1_constants

    robots.get_r1_robot_cfg = mjlab_r1_robot_cfg.get_r1_robot_cfg
    robots.R1_ACTION_SCALE = mjlab_r1_robot_cfg.R1_ACTION_SCALE
    r1_constants.get_r1_robot_cfg = mjlab_r1_robot_cfg.get_r1_robot_cfg
    r1_constants.R1_ACTION_SCALE = mjlab_r1_robot_cfg.R1_ACTION_SCALE


def _patch_flat_v2_task_registration() -> None:
    """Register local R1 task variants when upstream imports task configs."""
    from happy_baby_r1_training.mjlab_flat_v2_profile import (
        FLAT_V2_TASK_ID,
...
def main() -> None:
    ...
    sys.argv[0] = str(UPSTREAM_PLAY)
    runpy.run_path(str(UPSTREAM_PLAY), run_name="__main__")
