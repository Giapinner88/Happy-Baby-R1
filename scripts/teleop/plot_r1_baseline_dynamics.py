#!/usr/bin/env python3
"""Plot fixed-base root, joint and per-link COM traces from a baseline run."""
...
def _endpoint_tracking(run_dir: Path) -> dict[str, dict[str, np.ndarray]]:
    ...
    offline_continuation = bool(
        config.get("experiment_id") in {"baseline", "t007"}
        and "model" in config
    ...
    if int(config.get("schema_version", 0)) < 2 and not offline_continuation:
        ...
        # their mutable historical config. Applying the current tool frame to them
    ...
    if coupled:
        ...
        runtime = resolved.get("baseline_runtime", resolved.get("t007_runtime", {}))
        effective_mode = dict(runtime).get(
...
def main() -> int:
    ...
    parser.add_argument("run_dir", type=Path, help="Baseline run directory containing dynamics_trace.npz")
    ...
    if not trace_path.is_file(): raise SystemExit(f"Missing baseline dynamics trace: {trace_path}")
    ...
    if len(time_s) == 0: raise SystemExit("Baseline dynamics trace contains no samples.")
    ...
    if controller_rows:
        ...
        runtime = resolved.get("baseline_runtime", resolved.get("t007_runtime", {}))
        control_hz = float(dict(runtime).get("control_hz", 50.0))
